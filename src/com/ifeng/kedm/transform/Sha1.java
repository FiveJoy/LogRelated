...
public class Sha1 {
	static char CHARS[] = { '2', 'f', 'd', '3', '4', '0', 'e', '8', 'c', '9', '1', 'a', '7', 'b', '6', '5' };
	...
	public static String byteArrayToString(byte byteArray[], int arraySize) {
		StringBuffer temp = new StringBuffer();
		for (int i = 0; i < arraySize; i++) {
			if (i % 2 == 0) {
				temp.append(CHARS[(int) (((getUnsignInt(byteArray[i]) & 0x0FFFF) >> 4) & 0X0F)]);
				temp.append(CHARS[byteArray[i] & 0X0F]);
			}
		}
	...
	static long getUnsignInt(int num) {
		long unsignedValue = num & Integer.MAX_VALUE;
