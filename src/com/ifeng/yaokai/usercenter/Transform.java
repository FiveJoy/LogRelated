...
import com.ifeng.kedm.transform.Base32;
import com.ifeng.kedm.transform.Sha1;
import com.ifeng.kedm.transform.UrlDecode;
...
public class Transform {
	...
	public List<String> getAppStr(String str1) throws Throwable {
		...
		req = bodySplit[3];
		/*
		 * if(str1.contains("POST")){ String[] tempReq = bodySplit[5].split("\" \""); ip
		 * = tempReq[1].replace("\"",""); }
		 */
		...
		if ((splitUrl.get("datatype").equals("videoapp") && splitUrl.get("mos").contains("android")
				&& compareStrs(splitUrl.get("softversion"), "6.6.1"))
				|| (splitUrl.get("datatype").equals("newsapp") && splitUrl.get("mos").contains("android")
						&& compareStrs(splitUrl.get("softversion"), "4.1.1"))
				|| (splitUrl.get("datatype").equals("fmapp") && splitUrl.get("mos").contains("android")
						&& compareStrs(splitUrl.get("softversion"), "5.3.1"))) {
	...
	public Map<String, String> splitUrl(String req) {
		...
		/*
		 * String[] content = new String[50]; String strNew = req.replaceAll("&", "&&");
		 * Pattern pattern = Pattern.compile("[&?]+(.*?)(?=&|\\s|$)"); Matcher m =
		 * pattern.matcher(strNew); while(m.find()){ content[i]=m.group(1); i=i+1; }
		 * 
		 * for (String context : content) { if (context!=null) { String[] mapStrs =
		 * context.split("="); try { if (null == mapStrs[1]) { bodyMap.put(mapStrs[0],
		 * "#"); }else { bodyMap.put(mapStrs[0], mapStrs[1]); } } catch (Exception e) {
		 * e.printStackTrace(); } } }
		 */
